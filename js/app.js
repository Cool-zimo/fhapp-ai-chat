...
(function (global) {
    ...
    //  代码语法高亮（零依赖）
    //
    //  不引 highlight.js / Prism：那俩体积大且要额外加载 CSS，
    //  跟"零外部资源"的原则冲突。自研一个够用的子集。
    //
    //  ★ 安全：所有输出都过 escCode()，只转义 & < >。
    //    代码块内容放在**标签体内**而不是属性里，所以不用转义引号 ——
    //    这样字符串引号保持原样，高亮器才能识别它们。
    // ══════════════════════════════════════════════════════
    ...
    // 语言 → 规则
    //  kw: 关键字   com: [起,止] 注释对（止为 \n = 行注释）
    //  str: 字符串引号   tri: 支持三引号   html/css: 特殊模式
    var HL_SPEC = {
